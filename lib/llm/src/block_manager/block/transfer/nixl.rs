...
/// Maximum size for a single transfer entry in bytes
/// Can be increased for better performance with large contiguous transfers
    ...
    fn populate_xfer_desc_list(
        ...
        for (src, dst, size, src_device_id, dst_device_id) in self.descriptors.drain(..) {
            if let Some((last_src, last_dst, last_size, last_src_device_id, last_dst_device_id)) =
                &mut pending
            {
                // Can only merge if same device IDs, contiguous addresses, and won't exceed max size
                ...
                {
                    // Just increase the previous entry
                    *last_size += size;
                    continue;
                } else {
                    // Not contiguous, different devices, or would exceed max size - flush pending
        ...
        if let Some((last_src, last_dst, last_size, last_src_device_id, last_dst_device_id)) =
            pending
        {
            // Add left-over, chunking if necessary
            Self::add_chunked_descriptors(
                src_dl,
                dst_dl,
                last_src,
    ...
    Destination::StorageType: NixlDescriptor,
{
    ...
    // Collect transfer descriptors and aggregate them
    ...
    // Populate descriptor lists with aggregated and sorted descriptors
    // Device IDs are now extracted per descriptor in the aggregator
